...
class Database:
    ...
    @classmethod
    def insert_hotels(cls, temp_dict):
        cls.sql.execute(f"SELECT id FROM users WHERE user_id ={temp_dict['user_id']}")
        cls.db.commit()
        id = [tup[0] for tup in cls.sql.fetchall()][-1]
        cls.sql.execute(f"INSERT INTO hotels(request_id, name, price, address, distance, url, uploaded_at) "
                        f"VALUES (?,?,?,?,?,?,?)", (id,
                                                    temp_dict['name'], temp_dict['price'], temp_dict['address'],
                                                    temp_dict["distance"], temp_dict["url"], temp_dict['uploaded_at']))
        cls.db.commit()

    @classmethod
    def select_hotels(cls, user_id):

        cls.sql.execute(f"SELECT id FROM users WHERE user_id ={user_id}")
        cls.db.commit()
        id = [tup[0] for tup in cls.sql.fetchall()][-1]
        cls.sql.execute(f"SELECT name, price, address, distance, url, uploaded_at FROM hotels WHERE request_id = {id}")
        cls.db.commit()
